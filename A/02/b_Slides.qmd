...
::: {.fragment .mt1}
-   Additional Rules:
    -   Must [start with a letter]{.b .blue} (or a period not followed by a number)
...
an object called `age`. Letters, numbers, `_` and `.` are all a name may contain.
:::
...
::: {.takeaway}
These return a vector the same length as the one you gave them: one answer per
element.
:::
...
is a feature, not a bug. `na.rm = TRUE` says to answer from what you do have.
:::
