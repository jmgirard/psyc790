...
already exist --- which is how a script builds up from one line to the next.
:::
...
::: {.takeaway}
One capital letter makes an entirely different object. R will not warn you --- it
...
::: {.takeaway}
A function gives a name to something you would otherwise have to spell out ---
...
::: {.takeaway}
`c()` flattens whatever you give it into one vector --- there is no such thing as
...
::: {.takeaway}
`NA` means "we do not know", so any answer depending on it is also unknown ---
that is a feature, not a bug. `na.rm = TRUE` says to answer from what you do have.
:::
