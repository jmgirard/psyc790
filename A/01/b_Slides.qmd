...
::: aside
*Note.* Spaces around an operator are optional, but they make code easier to read.
:::
...
::: aside
*Note.* R has no `x` operator, so use `*` instead.
:::
...
::: aside
*Note.* The division slash leans the other way; `\` means something else entirely.
:::
...
::: aside
*Note.* Multiplication happens before addition; parentheses change the order.
:::
...
::: aside
*Note.* R drops leading and trailing zeros that carry no information.
:::
...
::: aside
*Note.* Separators are for humans, not for R — type the digits and nothing else.
:::
...
Comments are ignored by R and exist only for human readers. Start one with a hash
symbol.
...
100 / 9 # the start of the line still runs
```
