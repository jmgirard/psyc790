...
The grade column came in as `<chr>`, not because grades are letters, but because
one cell holds a `?`. R has no idea that means "missing."
...
::: aside
*Note.* Read the `NA` counts: 2 penguins were never measured and 11 more were never sexed. Real data has holes, and this is where you find them.
:::
