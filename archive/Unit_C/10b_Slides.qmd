...
## Venn diagram for simple regression {.f5}
...
## Venn diagram for two predictors {.f5}
...
## Partial effects of $x_1$ and $x_2$ {.f5}
...
## Standardized partial effects {.f5}
...
# install.packages(c("ggeffects", "effects"))
library(ggeffects)
```
...
plot(predict_response(fit3, "n_pubs")) + mytheme(18)
```
...
```{r, fig.width=5, fig.height = 3}
plot(predict_response(fit3, "yrs_since")) + mytheme(18)
```
:::
:::
...
```{r, fig.width=5, fig.height=3}
plot(predict_response(fit3, c("n_pubs", "yrs_since"))) + mytheme(18)
```
:::
::: {.column}
...
```{r, fig.width=5, fig.height = 3}
plot(predict_response(fit3, c("yrs_since", "n_pubs"))) + mytheme(18)
```
...
::: {.footer}
There are optional readings this week on this topic.
:::
