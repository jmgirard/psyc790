...
- Every variable that affects $y$ **and** correlates with an $x$ must be in the model
    + Such a variable is called a **confounder**
    + Its effect does not vanish; it is absorbed by the predictors we did include
...
::: {.callout-important title="Consequences of Violating"}
Coefficients stay **unbiased** but become unstable; standard errors are upwardly biased (i.e., inflated).
:::
...
1. Examine predictor correlations for collinearity
    ...
    + But this captures [c]{.b .green} and [d]{.b .purple}, whereas only [c]{.b .green} is an issue
...
## Example with Collinearity {.fs90}
...
## Example with Multicollinearity {.fs90}
