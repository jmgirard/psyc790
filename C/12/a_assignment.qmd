...
b) Of those three assumptions, which do you think is hardest to check in practice, and why?{{< hp glyph="b" >}}
...
a) Briefly report which partial effects are significant.
...
a) Fit the same model as in Question 2 (predicting `mental_wellbeing` from the four weekday screen-time variables plus `gender`, `minority`, and `deprived_area`). A reviewer is concerned that, because several predictors are all about "screen time," there may be multicollinearity. Check for this using the diagnostic(s) learned in lecture and interpret the results.

b) Now fit a second model that adds the four weekend predictors (`watch_we`, `play_we`, `compute_we`, `phone_we`). Re-check for multicollinearity. Which predictors became problematic, and why would adding weekend versions of the same activities do that?{{< hp glyph="b" >}}
