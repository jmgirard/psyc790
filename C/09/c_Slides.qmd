...
library(tidyverse)
screentime <- read_csv("../../data/screentime.csv")

fit3 <- lm(
  mental_wellbeing ~ play_wd + I(play_wd^2) + deprived_area,
  data = screentime
)
...
- The effects of `play_wd` are significant controlling for `deprived_area`

- The effect of `deprived_area` is *not* significant controlling for `play_wd` and its square
...
plot(estimate_relation(fit3, by = c("play_wd", "deprived_area")), show_data = TRUE)
```

```{r}
plot(estimate_relation(fit3, by = c("play_wd", "deprived_area")), show_data = TRUE) + mytheme() + labs(y = "mental_wellbeing")
```
...
fit4 <- lm(
  mental_wellbeing ~ play_wd + I(play_wd^2) + deprived_area +
    play_wd:deprived_area + I(play_wd^2):deprived_area,
  data = screentime
)
...
- The effects of `play_wd` are not moderated by `deprived_area`
...
plot(estimate_relation(fit4, by = c("play_wd", "deprived_area")))
```

```{r}
plot(estimate_relation(fit4, by = c("play_wd", "deprived_area"))) + mytheme() + labs(y = "mental_wellbeing")
```
